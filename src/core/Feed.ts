...
import type { FeedConfig, FeedStats, ValidationError, FeedFormat } from '../types/FeedTypes';
...
export class Feed {
   ...
   */
  getStats(): FeedStats {
    const totalImages = this.items.reduce((sum, item) => sum + (item.data.images?.length ?? 0), 0);

    const totalVideos = this.items.reduce((sum, item) => sum + (item.data.videos?.length ?? 0), 0);
    ...
    const averagePriority =
      priorities.length > 0 ? priorities.reduce((sum, p) => sum + p, 0) / priorities.length : 0;
  ...
  shouldSplit(): boolean {
    return (
      this.items.length > this.config.maxItems || this.estimateSize() > this.config.maxFileSize
    );
  }
  ...
  private hasMedia(): boolean {
    return this.items.some(
      item => (item.data.images?.length ?? 0) > 0 || (item.data.videos?.length ?? 0) > 0
    );
  }
